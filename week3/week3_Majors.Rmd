...
### First  version I'm going to create a vector whose elements are strings which contain strings
```{r}
...
quotedFoodVector
```
### second  i'm going to flatten the data into a single string that contains the source code to create a vector in R:
```{r}
...
* (.)\1\1
The single backslash is eaten by the string before it gets to the reg-ex interpreter.

I believe it matches octal ascii code \001, "start of header" which I can't find documented too well: [https://www.ascii-code.com/character/%E2%90%81](https://www.ascii-code.com/character/%E2%90%81)
